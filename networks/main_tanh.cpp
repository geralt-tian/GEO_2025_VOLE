...
int init_test(uint64_t i, uint64_t j, uint64_t k, uint64_t l)
{
    ...
    if (party == sci::ALICE)
    {
        std::ifstream file("/root/Cheetah/scripts/tanh_la_ld_s6.csv");
        if (!file.is_open())
        {
    ...
    uint64_t comm_start = io->counter;
    std::cout << "init LinearOT" << std::endl;
    ...
    if (party == sci::ALICE)
    {
        trunc_oracle->truncate_and_reduce(dim, EMUX_output_x1_h, outtrunc, h - s, h); // step 11 tr
    ...
    if (correct == 1)
    {
        if (party == sci::ALICE)
        {
            ...
            sirnn_unsigned_mul(dim, a_alice, EMUX_output_x, outax, la, bwL, la + bwL);
        ...
        else
        {
            ...
            sirnn_unsigned_mul(dim, a_bob, EMUX_output_x, outax, la, bwL, la + bwL);
    ...
    uint64_t STEP14_comm_start = io->counter;
    uint64_t STEP14_comm_end = io->counter;
    ...
    select_share(Drelu_,z, one_f, z_or_one, dim, bwL);
    ...
    if (acc == 2)
    {
        ...
        select_share(Drelu, z_or_one, neg_z, di_abs_z_or_one, dim, bwL); // step 21 ss
    }
    ...
    else
    {
        ...
        for (int i = 0; i < dim; i++)
        {
            ...
            ULPs[i] = computeULPErr(((double)decode_ring((y[i] + recv_y[i]) & mask_bwL, bwL) / f_pow), calculate_tanh(inA[i] + inB[i],12), s_y);
...
int main(int argc, char **argv)
{
    ...
    std::vector<std::pair<uint64_t, uint64_t>> la_lb_pairs = {
        {7,13},{7,12},{6,12},{6,11},{5,11},{5,10},{4,10},{4,9}};
