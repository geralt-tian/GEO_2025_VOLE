...
int main(int argc, char **argv) {
    ...
    gp = new GeometricPerspectiveProtocols(party, io, otpack);
    ...
    if (party == sci::ALICE) {
        io->send_data(result, dim * sizeof(uint64_t));
        io->send_data(inA, dim * sizeof(uint64_t));
        io->recv_data(result_bob, dim * sizeof(uint64_t));
        io->recv_data(input_bob, dim * sizeof(uint64_t));
        memcpy(result_alice, result, dim * sizeof(uint64_t));
        memcpy(input_alice, inA, dim * sizeof(uint64_t));
    } else {
        io->recv_data(result_alice, dim * sizeof(uint64_t));
        io->recv_data(input_alice, dim * sizeof(uint64_t));
        io->send_data(result, dim * sizeof(uint64_t));
        io->send_data(inA, dim * sizeof(uint64_t));
