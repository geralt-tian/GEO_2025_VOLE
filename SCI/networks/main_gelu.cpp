...
int init_test(uint64_t i, uint64_t j, uint64_t k, uint64_t l)
{
    ...
    if (party == sci::ALICE)
    {
        std::ifstream file("/root/Cheetah/scripts/gelu_la_ld_s6.csv");
        if (!file.is_open())
        {
    ...
    if (correct == 1)
    {
        if (party == sci::ALICE)
        {
            ...
            sirnn_signed_mul(dim, a_alice, EMUX_output_x, outax, la, bwL, la + bwL);
        ...
        else
        {
            ...
            sirnn_signed_mul(dim, a_bob, EMUX_output_x, outax, la, bwL, la + bwL);
...
int main(int argc, char **argv)
{
    ...
    std::vector<std::pair<uint64_t, uint64_t>> la_lb_pairs = {
        {8,13},{4,9}};
        // {8,13},{6,13},{6,12},{6,11},{5,11},{4,11},{4,10},{4,9}};
